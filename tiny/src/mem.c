...
#ifdef TINY_MEM_CHECK
const tiny_allocation_record *tiny_mem_get_allocations(void) { return record_list; }
#endif
    ...
    while (node) {
        ...
        for (int i = 0; i < TINY_MEM_STACK_TRACE_COUNT; ++i) {
            if (SymFromAddr(proc, (DWORD64)node->callstack[i], NULL, p_symbol)) {
                ...
                DWORD error = GetLastError();
                if (error != 126) fprintf(stderr, "SymFromAddr failed with error code %d\n", error);
            }
        }
#endif
...
static void print_stack_trace(void) {
    ...
    for (int i = 0; i < numFrames; ++i) {
        ...
        if (SymGetLineFromAddr64(process, (DWORD64)(frames[i]), &displacement, &line))
            fprintf(stderr, "%i: %s(%i): %s - 0x%0llX\n", i, line.FileName, (int)line.LineNumber,
...
#ifdef _WIN32
            USHORT count =
                CaptureStackBackTrace(0, TINY_MEM_STACK_TRACE_COUNT, record->callstack, NULL);
#else
                        memset(record->callstack, 0, sizeof(record->callstack);
#endif
