...
inline static void GenOp(Tiny_State* state, uint8_t op)
{
    BUF_PUSH(state->code, op);
}
...
inline static void GenInt(Tiny_State* state, int i)
{
    GenBytes(state, alignof(i), &i, sizeof(i));
}
