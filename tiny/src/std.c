...
const Tiny_NativeProp ArrayProp = { "array", ArrayMark, ArrayFree, };
...
static Tiny_Value CreateDict(Tiny_StateThread *thread, const Tiny_Value *args, int count) {
    ...
    if (count > 0 && count % 2 != 0) {
        fprintf(stderr, "Expected even number of arguments to dict(...) (since each key "
                "needs a corresponding value) but got %d.\n", count);
        exit(1);
    }

    for (int i = 0; i < count; i += 2)
        DictSet(dict, args[i], args[i + 1]);
...
static Tiny_MacroResult BindJsonSerializerForType(Tiny_State *state, const Tiny_Symbol *sym) {
                ...
                return (Tiny_MacroResult ) { .type = TINY_MACRO_SUCCESS } ;
                    ...
                    if (Tiny_FindFuncSymbol(state, buf)) {
                        ...
                        return (Tiny_MacroResult ) { .type = TINY_MACRO_SUCCESS } ;
                            ...
                            used += snprintf(buf, sizeof(buf), "func %s_to_json(v: %s): str {\n\treturn strcat(\"{\",\n\t\t", sym->name, sym->name);
                            ...
                            for (int i = 0; i < Tiny_SymbolArrayCount(sym->sstruct.fields); ++i) {
                                ...
                                used += snprintf(buf + used, sizeof(buf) - used, "\"\\\"%s\\\":\", %s_to_json(v.%s)", fieldSym->name, fieldSym->fieldTag->name,
                                        fieldSym->name);
                            }
                            ...
                            return (Tiny_MacroResult ) { .type = TINY_MACRO_SUCCESS } ;
                                ...
                                static TINY_MACRO_FUNCTION(JsonMacroFunction) {
                                    if (nargs != 1) {
                                        return (Tiny_MacroResult ) { .type = TINY_MACRO_ERROR, .errorMessage = "Must specify exactly 1 argument to 'use json'", } ;
                                            }

                                            const Tiny_Symbol *sym = Tiny_FindTypeSymbol(state, args[0]);

                                            if (sym->type != TINY_SYM_TAG_STRUCT) {
                                                return (Tiny_MacroResult ) { .type = TINY_MACRO_ERROR, .errorMessage =
                                                                "Must specify struct type as argument to 'use json_mod'", } ;
                                                    }
                                                ...
                                                static TINY_MACRO_FUNCTION(ArrayMacroFunction) {
                                                    if (nargs != 1) {
                                                        return (Tiny_MacroResult ) { .type = TINY_MACRO_ERROR, .errorMessage =
                                                                        "Must specify exactly 1 argument to 'use array'", } ;
                                                            }

                                                            if (!asName) {
                                                                return (Tiny_MacroResult ) { .type = TINY_MACRO_ERROR, .errorMessage =
                                                                                "Must specify an 'as' name when doing 'use array'", } ;
                                                                    }

                                                                    if (!Tiny_FindTypeSymbol(state, args[0])) {
                                                                        return (Tiny_MacroResult ) { .type = TINY_MACRO_ERROR, .errorMessage =
                                                                                        "The array element type you specified does not exist", } ;
                                                                            }
                                                                            ...
                                                                            snprintf(sigbuf, sizeof(sigbuf), "%s_set(%s, int, %s): void", asName, asName,
                                                                                    args[0]);
                                                                            ...
                                                                            return (Tiny_MacroResult ) { .type = TINY_MACRO_SUCCESS } ;
                                                                                ...
                                                                                void Tiny_BindStandardDict(Tiny_State *state) {
                                                                                    ...
                                                                                    Tiny_BindFunction(state, "dict_str_int_put(dict_str_int, str, int): void",
                                                                                            Lib_DictPut);
                                                                                    Tiny_BindFunction(state, "dict_str_int_exists(dict_str_int, str): bool",
                                                                                            Lib_DictExists);
                                                                                    Tiny_BindFunction(state, "dict_str_int_get(dict_str_int, str): int",
                                                                                            Lib_DictGet);
                                                                                    Tiny_BindFunction(state, "dict_str_int_remove(dict_str_int, str): void",
                                                                                            Lib_DictRemove);
                                                                                    Tiny_BindFunction(state, "dict_str_int_keys(dict_str_int): array_str",
                                                                                            Lib_DictKeys);
                                                                                    Tiny_BindFunction(state, "dict_str_int_clear(dict_str_int): void",
                                                                                            Lib_DictClear);
                                                                                }
                                                                                ...
                                                                                static TINY_MACRO_FUNCTION(DelegateMacroFunction) {
                                                                                    if (nargs != 1) {
                                                                                        return (Tiny_MacroResult ) { .type = TINY_MACRO_ERROR, .errorMessage =
                                                                                                        "Must specify exactly 1 argument to 'use delegate'", } ;
                                                                                            }

                                                                                            if (!asName) {
                                                                                                return (Tiny_MacroResult ) { .type = TINY_MACRO_ERROR,
                                                                                                                .errorMessage =
                                                                                                                        "Must specify an 'as' name when doing 'use delegate'", } ;
                                                                                                    }
                                                                                                    ...
                                                                                                    const Tiny_Symbol *func = Tiny_FindFuncSymbol(state,
                                                                                                            args[0]);

                                                                                                    for (int i = 0;
                                                                                                            i
                                                                                                                    < Tiny_SymbolArrayCount(
                                                                                                                            func->type == TINY_SYM_FUNCTION ?
                                                                                                                                    func->func.args :
                                                                                                                                    func->foreignFunc.argTags);
                                                                                                            ++i) {
                                                                                                        if (i > 0) {
                                                                                                            typeUsed += snprintf(typeBuf + typeUsed,
                                                                                                                    sizeof(typeBuf) - typeUsed, "_");
                                                                                                            argsUsed += snprintf(argsBuf + argsUsed,
                                                                                                                    sizeof(argsBuf) - argsUsed, ", ");
                                                                                                            paramsUsed += snprintf(paramsBuf + paramsUsed,
                                                                                                                    sizeof(paramsBuf) - paramsUsed, ", ");
                                                                                                        }

                                                                                                        const Tiny_Symbol *argTag =
                                                                                                                func->type == TINY_SYM_FUNCTION ?
                                                                                                                        func->func.args[i]->var.tag :
                                                                                                                        func->foreignFunc.argTags[i];

                                                                                                        typeUsed += snprintf(typeBuf + typeUsed,
                                                                                                                sizeof(typeBuf) - typeUsed, "%s", argTag->name);
                                                                                                        argsUsed += snprintf(argsBuf + argsUsed,
                                                                                                                sizeof(argsBuf) - argsUsed, "a%d: %s", i,
                                                                                                                argTag->name);
                                                                                                        paramsUsed += snprintf(paramsBuf + paramsUsed,
                                                                                                                sizeof(paramsBuf) - paramsUsed, "a%d", i);
                                                                                                    }

                                                                                                    const Tiny_Symbol *returnTag =
                                                                                                            func->type == TINY_SYM_FUNCTION ?
                                                                                                                    func->func.returnTag :
                                                                                                                    func->foreignFunc.returnTag;

                                                                                                    const char *returnTagName = returnTag->name;

                                                                                                    typeUsed += snprintf(typeBuf + typeUsed,
                                                                                                            sizeof(typeBuf) - typeUsed, "_%s", returnTagName);
                                                                                                    ...
                                                                                                    if (!Tiny_FindTypeSymbol(state, typeBuf)) {
                                                                                                        ...
                                                                                                        if (returnTag->type != TINY_SYM_TAG_VOID) {
                                                                                                            snprintf(callBuf, sizeof(callBuf),
                                                                                                                    ...
                                                                                                                    typeBuf, typeBuf, argsBuf, returnTagName,
                                                                                                                    paramsBuf, returnTagName);
                                                                                                        } else {
                                                                                                            snprintf(callBuf, sizeof(callBuf),
                                                                                                                    "func %s_call(f: %s, %s) { call_function(f, %s) }",
                                                                                                                    typeBuf, typeBuf, argsBuf, paramsBuf);
                                                                                                        }

                                                                                                        Tiny_CompileString(state, "(delegate call function)",
                                                                                                                callBuf);
                                                                                                    }
                                                                                                    ...
                                                                                                    if (!Tiny_FindFuncSymbol(state, asName)) {
                                                                                                        ...
                                                                                                        snprintf(createBuf, sizeof(createBuf),
                                                                                                                "func %s(): %s { return cast(get_function_index(\"%s\"), %s) }",
                                                                                                                asName, typeBuf, args[0], typeBuf);

                                                                                                        Tiny_CompileString(state, "(delegate create function",
                                                                                                                createBuf);
                                                                                                    }
                                                                                                    ...
                                                                                                    return (Tiny_MacroResult ) { .type = TINY_MACRO_SUCCESS } ;
                                                                                                        ...
                                                                                                        static TINY_FOREIGN_FUNCTION(GetFunctionIndex) {
                                                                                                            ...
                                                                                                            int i = Tiny_GetFunctionIndex(thread->state,
                                                                                                                    Tiny_ToString(args[0]));
                                                                                                        ...
                                                                                                        static TINY_FOREIGN_FUNCTION(CallFunction) {
                                                                                                            ...
                                                                                                            return Tiny_CallFunction(thread, i, args + 1,
                                                                                                                    count - 1);
                                                                                                        }
                                                                                                        ...
                                                                                                        void Tiny_BindStandardLib(Tiny_State *state) {
                                                                                                            ...
                                                                                                            Tiny_BindFunction(state, "strlen(str): int",
                                                                                                                    Strlen);
                                                                                                            Tiny_BindFunction(state, "stridx(str, int): int",
                                                                                                                    Stridx);
                                                                                                            Tiny_BindFunction(state, "sget(str, int): int",
                                                                                                                    Stridx);
                                                                                                            Tiny_BindFunction(state, "strchr(str, int): int",
                                                                                                                    Strchr);
                                                                                                            Tiny_BindFunction(state,
                                                                                                                    "strcat(str, str, ...): str", Strcat);
                                                                                                            Tiny_BindFunction(state,
                                                                                                                    "substr(str, int, int): str", Lib_Substr);

                                                                                                            Tiny_BindFunction(state, "ston(str): float",
                                                                                                                    Lib_Ston);
                                                                                                            Tiny_BindFunction(state, "str_to_int(str): int",
                                                                                                                    Lib_Stoi);
                                                                                                            Tiny_BindFunction(state, "ntos(...): str",
                                                                                                                    Lib_Ntos);

                                                                                                            Tiny_BindFunction(state, "stoi(str, int): int",
                                                                                                                    Lib_Stoi);

                                                                                                            Tiny_BindFunction(state, "time(): int", Lib_Time);
                                                                                                            Tiny_BindFunction(state, "srand(int): void",
                                                                                                                    SeedRand);
                                                                                                            Tiny_BindFunction(state, "rand(): int", Rand);

                                                                                                            Tiny_BindFunction(state, "floor(float): float",
                                                                                                                    Lib_Floor);
                                                                                                            Tiny_BindFunction(state, "ceil(float): float",
                                                                                                                    Lib_Ceil);

                                                                                                            // HACK(Apaar): These are labeled as returning/taking any to facilitate the conversion to
                                                                                                            // a delegate
                                                                                                            Tiny_BindFunction(state,
                                                                                                                    "get_function_index(str): any",
                                                                                                                    GetFunctionIndex);
                                                                                                            Tiny_BindFunction(state,
                                                                                                                    "call_function(any, ...): any",
                                                                                                                    CallFunction);

                                                                                                            Tiny_BindMacro(state, "delegate",
                                                                                                                    DelegateMacroFunction);
    ...
    Tiny_BindFunction(state, "sleep(int): void", Lib_Sleep);
                                                                                                            ...
                                                                                                            Tiny_BindFunction(state, "bool_to_json",
                                                                                                                    Lib_PrimitiveToJson);
                                                                                                            Tiny_BindFunction(state, "str_to_json",
                                                                                                                    Lib_PrimitiveToJson);
                                                                                                            Tiny_BindFunction(state, "int_to_json",
                                                                                                                    Lib_PrimitiveToJson);
                                                                                                            Tiny_BindFunction(state, "float_to_json",
                                                                                                                    Lib_PrimitiveToJson);

                                                                                                            Tiny_BindFunction(state, "get_executing_line",
                                                                                                                    GetExecutingLine);
