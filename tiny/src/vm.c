...
static void Mark(Tiny_VM* vm, Tiny_Object* obj) {
    ...
    switch (obj->type) {
        case TINY_VAL_STRING:
            Tiny_StringPoolRetain(vm->stringPool, obj->s);
            break;
        default:
            break;
    }
...
static void DeleteObject(Tiny_VM* vm, Tiny_Object* obj) {
    ...
    switch (obj->type) {
        case TINY_VAL_STRING:
            Tiny_StringPoolRelease(vm->stringPool, obj->s);
            break;
        default:
            break;
    }
...
static void PushValue(Tiny_VM* vm, Tiny_Value value) { *vm->sp++ = value; }

static Tiny_Value PopValue(Tiny_VM* vm) { return *(--vm->sp); }
...
#define INT_OP(name, operator) ARITH_OP(i, name, operator, PushInt)

#define INT_CMP_OP(name, operator) ARITH_OP(i, name, operator, PushBool)

#define FLOAT_OP(name, operator) ARITH_OP(f, name, operator, PushFloat)

#define FLOAT_CMP_OP(name, operator) ARITH_OP(f, name, operator, PushBool)

#define BOOL_OP(name, operator) ARITH_OP(b, name, operator, PushBool)

#define BOOL_CMP_OP(name, operator) ARITH_OP(b, name, operator, PushBool)

#define CHAR_CMP_OP(name, operator) ARITH_OP(c, name, operator, PushChar)
