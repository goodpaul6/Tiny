...
static void ResolveTypes(void* data, AST* ast)
{
    ...
    switch(ast->type) {
        ...
        case AST_CALL: {
            ...
            if(!funcType->func.varargs && 
                BUF_LEN(ast->call.args) > BUF_LEN(funcType->func.args)) {
                RESOLVER_ERROR_AST(r, ast, "Too many arguments to function '%s'; expected %d.", ast->call.calleeName, BUF_LEN(funcType->func.args));
            }

            for(int i = 0; i < BUF_LEN(funcType->func.args); ++i) {
                if(!CompareTypes(ast->call.args[i], funcType->func.args[i])) {
                    RESOLVER_ERROR_AST(r, ast->call.args[i], "Argument %i is supposed to be a %s but you supplied a %s.\n", GetTypeName(r->sym, ast->call.args[i]->tag), GetTypeName(r->sym, funcType->func.args[i]));
                }
            }
        ...
        case AST_BINARY: {
            switch(ast->binary.op) {
                case TOK_PLUS: case TOK_MINUS: case TOK_STAR: case TOK_SLASH: {
                    ...
                    ast->tag = GetPrimitiveTypetag(r->tp, 
                            (iLhs && iRhs) ? TYPETAG_INT : TYPETAG_FLOAT);
                } break;
                ...
                case TOK_AND: case TOK_OR: case TOK_PERCENT: {
                    ...
                    if(!(iLhs && iRhs)) {
                        RESOLVER_ERROR_AST(r, ast, "Both sides of binary operation must be ints, but they're %s and %s.",
                                GetTypeName(r->sym, ast->binary.lhs->tag),
                                GetTypeName(r->sym, ast->binary.rhs->tag));
                    }
                ...
                case TOK_LOG_AND: case TOK_LOG_OR: {
                    ...
                    if(!bLhs || !bRhs) {
                        RESOLVER_ERROR_AST(r, ast, "Both sides of the binary operator must be bools, but they're %s and %s.",
                                GetTypeName(r->sym, ast->binary.lhs->tag),
                                GetTypeName(r->sym, ast->binary.rhs->tag));
                    }
                ...
                case TOK_DECLARE: {
                    ...
                    if(ast->binary.rhs->tag->type == TYPETAG_VOID) {
                        RESOLVER_ERROR_AST(r, ast, "Attempted to initialize variable with void expression. Don't do that.");
                    }
        ...
        case AST_UNARY: {
            ...
            switch(ast->unary.op) {
                case TOK_MINUS: {
                    ...
                    if(!(i || f)) {
                        RESOLVER_ERROR_AST(r, ast, "Attempted to apply unary '-' to a %s.", GetTypeName(ast->unary.exp->tag));
                    }
        ...
        case AST_IF: {
            ...
            if(exp->ifx.cond->tag->type != TYPETAG_BOOL) {
                RESOLVER_ERROR_AST(r, ast->ifx.cond, "If condition is supposed to be a bool but it's a %s.", GetTypeName(r->sym, ast->ifx.cond->tag));
            }
        ...
        case AST_WHILE: {
            ...
            if(ast->whilex.cond->tag->type != TYPETAG_BOOL) {
                RESOLVER_ERROR_AST(r, ast, "While condition is supposed to be a bool but it's a %s.", GetTypeName(r->sym, ast->whilex.cond->tag));
            }
        ...
        case AST_FOR: {
            ...
            if(ast->forx.cond->tag->type != TYPETAG_BOOL) {
                RESOLVER_ERROR_AST(r, ast, "For condition is supposed to be a bool but it's a %s.", GetTypeName(r->sym, ast->forx.cond->tag));
            }
        ...
        case AST_DOT: {
            ...
            if(ast->dot.lhs->tag->type != TYPETAG_STRUCT) {
                RESOLVER_ERROR_AST(r, ast, "Cannot use '.' on a %s.", GetTypeName(r->sym, ast->dot.lhs->tag));
            }

            int i = GetFieldIndex(ast->dot.lhs->tag, ast->dot.field);
            
            if(i < 0) {
                RESOLVER_ERROR_AST(r, ast, "Struct %s does not have a field named %s.", GetTypeName(r->sym, ast->dot.lhs->tag), ast->dot.field);
            }        
        ...
        case AST_CONSTRUCTOR: {
            ...
            if(meCount != tagCount) {
                RESOLVER_ERROR_AST(r, ast, "Struct %s constructor expects %d args but you supplied %d.", ast->constructor.structSym->name, tagCount, meCount);
            }

            for(int i = 0; i < BUF_LEN(ast->constructor.args); ++i) {
                ResolveTypes(r, ast->constructor.args[i]);

                if(!CompareTypes(ast->constructor.args[i], ast->constructor.structSym->typetag->tstruct.types[i])) {
                    RESOLVER_ERROR_AST(r, ast, "Argument %d to constructor is supposed to be a %s but you supplied a %s.", i + 1, 
        ...
        case AST_CAST: {
            ...
            if(ast->cast.value->tag->type != TYPETAG_ANY) {
                RESOLVER_ERROR_AST(r, ast->cast.value, "Attempted to cast a %s; only any is allowed.", GetTypeName(r->sym, ast->cast.value->tag));
            }
